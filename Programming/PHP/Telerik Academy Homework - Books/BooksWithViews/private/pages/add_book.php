...
$data['nav'] = $base_path . $file_loc['html_snippet']['nav_logged']['add_book'];
...
$data['content'] = $base_path . $file_loc['html_snippet']['form']['add_book'];
$data['send_form_to'] = $base_path . $file_loc['script']['add_book'];

$data['authors'] = get_authors($error, $db_conn, 'acs');
...
if (!$error) {
    ...
    if (isset($_GET['added'])) {
        ...
        if ($is_author_added === 'yes') {
            ...
            $data['message'] = 'A new book has been added!';
        }
    }
}
 else {
