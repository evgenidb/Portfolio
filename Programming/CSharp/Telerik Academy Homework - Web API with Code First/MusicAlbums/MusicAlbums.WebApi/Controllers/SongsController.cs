using System;
using System.Collections.Generic;
using System.Data;
using System.Data.Entity;
using System.Data.Entity.Infrastructure;
using System.Linq;
using System.Net;
using System.Net.Http;
using System.Web;
using System.Web.Http;
using MusicAlbums.Model;
using MusicAlbums.Data;
using MusicAlbums.Repositories;

namespace MusicAlbums.WebApi.Controllers
{
    public class SongsController : ApiController
    {
        //private MusicAlbumsContext db = new MusicAlbumsContext();
        private readonly IRepository<Song> songsRepository;

        public SongsController()
        {
            var dbContext = new MusicAlbumsContext();
            this.songsRepository = new DbSongsRepository(dbContext);
        }

        public SongsController(IRepository<Song> repository)
        {
            this.songsRepository = repository;
        }

        // GET api/Songs
        public IQueryable<Song> GetSongs()
        {
            //var songs = db.Songs.Include(s => s.Artist);
            //return songs.AsEnumerable();

            return this.songsRepository.GetAllFull();
        }

        // GET api/Songs/5
        public Song GetSong(int id)
        {
            //Song song = db.Songs.Find(id);
            //if (song == null)
            //{
            //    throw new HttpResponseException(Request.CreateResponse(HttpStatusCode.NotFound));
            //}

            //return song;

            var song = this.songsRepository.Get(id);
            if (song == null)
            {
                throw new HttpResponseException(Request.CreateResponse(HttpStatusCode.NotFound, "No Song with such Id."));
            }

            return song;
        }

        // PUT api/Songs/5
        public HttpResponseMessage PutSong(int id, Song song)
        {
            //if (!ModelState.IsValid)
            //{
            //    return Request.CreateErrorResponse(HttpStatusCode.BadRequest, ModelState);
            //}

            //if (id != song.Id)
            //{
            //    return Request.CreateResponse(HttpStatusCode.BadRequest);
            //}

            //db.Entry(song).State = EntityState.Modified;

            //try
            //{
            //    db.SaveChanges();
            //}
            //catch (DbUpdateConcurrencyException ex)
            //{
            //    return Request.CreateErrorResponse(HttpStatusCode.NotFound, ex);
            //}

            //return Request.CreateResponse(HttpStatusCode.OK);

            try
            {
                this.songsRepository.UpdateAndSave(id, song);
                return Request.CreateResponse(HttpStatusCode.OK);
            }
            catch (ArgumentNullException ex)
            {
                return Request.CreateResponse(HttpStatusCode.BadRequest, ex.Message);
            }
            catch (ArgumentException ex)
            {
                return Request.CreateResponse(HttpStatusCode.BadRequest, ex.Message);
            }
        }

        // POST api/Songs
        public HttpResponseMessage PostSong(Song song)
        {
            //if (ModelState.IsValid)
            //{
            //    db.Songs.Add(song);
            //    db.SaveChanges();

            //    HttpResponseMessage response = Request.CreateResponse(HttpStatusCode.Created, song);
            //    response.Headers.Location = new Uri(Url.Link("DefaultApi", new { id = song.Id }));
            //    return response;
            //}
            //else
            //{
            //    return Request.CreateErrorResponse(HttpStatusCode.BadRequest, ModelState);
            //}

            this.songsRepository.AddAndSave(song);

            HttpResponseMessage response = Request.CreateResponse(HttpStatusCode.Created, song);
            response.Headers.Location = new Uri(Url.Link("DefaultApi", new { id = song.Id }));
            return response;
        }

        // DELETE api/Songs/5
        public HttpResponseMessage DeleteSong(int id)
        {
            //Song song = db.Songs.Find(id);
            //if (song == null)
            //{
            //    return Request.CreateResponse(HttpStatusCode.NotFound);
            //}

            //db.Songs.Remove(song);

            //try
            //{
            //    db.SaveChanges();
            //}
            //catch (DbUpdateConcurrencyException ex)
            //{
            //    return Request.CreateErrorResponse(HttpStatusCode.NotFound, ex);
            //}

            //return Request.CreateResponse(HttpStatusCode.OK, song);

            try
            {
                var song = this.songsRepository.DeleteAndSave(id);
                return Request.CreateResponse(HttpStatusCode.OK, song);
            }
            catch (NullReferenceException ex)
            {
                return Request.CreateResponse(HttpStatusCode.NotFound, ex.Message);
            }
        }

        protected override void Dispose(bool disposing)
        {
            //db.Dispose();
            this.songsRepository.Dispose();
            base.Dispose(disposing);
        }
    }
}